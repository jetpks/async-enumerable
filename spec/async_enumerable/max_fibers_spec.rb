...
RSpec.describe "AsyncEnumerable max_fibers" do
  describe "module-level configuration" do
    it "has a default max_fibers value of 1024" do
      expect(AsyncEnumerable.max_fibers).to eq(1024)
    end

    it "allows setting max_fibers value" do
      original = AsyncEnumerable.max_fibers
      AsyncEnumerable.max_fibers = 50
      expect(AsyncEnumerable.max_fibers).to eq(50)
      AsyncEnumerable.max_fibers = original
    end
  end
  ...
  describe "per-instance configuration" do
    it "accepts max_fibers parameter in async method" do
      ...
      expect(enum).to be_a(AsyncEnumerable::AsyncEnumerator)
    end
