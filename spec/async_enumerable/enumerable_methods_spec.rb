...
RSpec.describe "Enumerable methods with async each" do
  ...
  describe "predicates" do
    it "all? works with async each" do
      expect(async_array.all? { |n| n > 0 }).to be true
      expect(async_array.all? { |n| n > 3 }).to be false
    end

    it "any? works with async each" do
      expect(async_array.any? { |n| n > 3 }).to be true
      expect(async_array.any? { |n| n > 10 }).to be false
    end

    it "none? works with async each" do
      expect(async_array.none? { |n| n > 10 }).to be true
      expect(async_array.none? { |n| n > 3 }).to be false
    end

    it "one? works with async each" do
      expect(async_array.one? { |n| n == 3 }).to be true
      expect(async_array.one? { |n| n > 3 }).to be false
    end

    it "include? works with async each" do
      expect(async_array.include?(3)).to be true
      expect(async_array.include?(10)).to be false
    end

    it "member? works with async each" do
      expect(async_array.member?(3)).to be true
      expect(async_array.member?(10)).to be false
    end
  end

  describe "filtering" do
    it "select works with async each" do
      result = async_array.select { |n| n > 2 }
      expect(result).to eq([3, 4, 5])
    end

    it "reject works with async each" do
      result = async_array.reject { |n| n > 2 }
      expect(result).to eq([1, 2])
    end

    it "find works with async each" do
      result = async_array.find { |n| n > 2 }
      expect(result).to eq(3)
    end

    it "detect works with async each" do
      result = async_array.detect { |n| n > 2 }
      expect(result).to eq(3)
    end

    it "find_all works with async each" do
      result = async_array.find_all { |n| n > 2 }
      expect(result).to eq([3, 4, 5])
    end

    it "filter works with async each" do
      result = async_array.filter { |n| n > 2 }
      expect(result).to eq([3, 4, 5])
    end

    it "filter_map works with async each" do
      result = async_array.filter_map { |n| n * 2 if n > 2 }
      expect(result).to eq([6, 8, 10])
    end

    it "grep works with async each" do
      strings = ["apple", "banana", "cherry"].async
      result = strings.grep(/a/)
      expect(result).to eq(["apple", "banana"])
    end

    it "grep_v works with async each" do
      strings = ["apple", "banana", "cherry"].async
      result = strings.grep_v(/a/)
      expect(result).to eq(["cherry"])
    end

    it "partition works with async each" do
  ...
  describe "aggregation" do
    it "reduce works with async each" do
    ...
    it "inject works with async each" do
      result = async_array.inject { |sum, n| sum + n }
      expect(result).to eq(15)
    end

    it "sum works with async each" do
  ...
  describe "slicing and taking" do
    it "first works with async each" do
      expect(async_array.first).to eq(1)
      expect(async_array.first(3)).to eq([1, 2, 3])
    end

    it "take works with async each" do
      expect(async_array.take(3)).to eq([1, 2, 3])
    end

    it "take_while works with async each" do
      result = async_array.take_while { |n| n < 4 }
      expect(result).to eq([1, 2, 3])
    end

    it "drop works with async each" do
      expect(async_array.drop(2)).to eq([3, 4, 5])
    end

    it "drop_while works with async each" do
  ...
  describe "conversion" do
    it "to_a works with async each" do
      expect(async_array.to_a).to eq([1, 2, 3, 4, 5])
    end

    it "entries works with async each" do
      expect(async_array.entries).to eq([1, 2, 3, 4, 5])
    end

    it "to_h works with async each" do
      pairs = [[1, "one"], [2, "two"], [3, "three"]].async
      expect(pairs.to_h).to eq({1 => "one", 2 => "two", 3 => "three"})
    end

    it "to_set works with async each" do
      require "set"
  ...
  describe "finding" do
    it "find_index works with async each" do
      expect(async_array.find_index(3)).to eq(2)
      expect(async_array.find_index { |n| n > 3 }).to eq(3)
    end

    it "each_entry works with async each" do
      result = []
